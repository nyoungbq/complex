...
TEST_CASE("OrientationAnalysis::FindFeatureReferenceMisorientationsFilter: Constant Misorientation Valid Test", "[OrientationAnalysis][FindFeatureReferenceMisorientationsFilter]")
{
  Application::GetOrCreateInstance()->loadPlugins(unit_test::k_BuildDir.view(), true);
  ...
  // Instantiate the filter and an Arguments Object
  {
    ...
    args.insertOrAssign(FindFeatureReferenceMisorientationsFilter::k_ConstantRefOrientationVec_Key, std::make_any<std::vector<float32>>(std::vector<float32>{0,0,0,0}));
    args.insertOrAssign(FindFeatureReferenceMisorientationsFilter::k_GBEuclideanDistancesArrayPath_Key, std::make_any<DataPath>(DataPath{}));
